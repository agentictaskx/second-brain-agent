...
Before returning results (see `references/completion-checklist.md`):
- [ ] Raw source saved
...
See `references/tool-fallback-chains.md` for fallback order.
...
- **P6:** Schema is discipline — this file defines the rules
- **P7:** Maintenance cost approaches zero — more agents ≠ better
- **P8:** Index-first navigation — always read `index.md` before drilling into pages
- **P9:** Lint keeps wiki healthy — regular audits catch rot
- **P10:** Connections = content — cross-links between pages are knowledge

## Bootstrap Detection
If `wiki/identity.md` does not exist → enter bootstrap mode:
1. Interview user for name, role, team, org
2. Create `wiki/identity.md`
3. Ask for key stakeholders → create `wiki/people.md`
4. Ask for monitored channels → create `wiki/subscriptions.md`
5. Ask for top priorities → create `wiki/top-of-mind.md` + `wiki/todo.md`
